...
このライブラリは、swim-live結果データにアクセスするための堅牢で型安全かつ正規化されたインターフェースを提供します。

## 目次

1. [インストール](#インストール)
2. [基本的な使い方](#基本的な使い方)
3. [APIメソッド一覧](#apiメソッド一覧)
   - [大会関連API](#大会関連api)
   - [レース関連API](#レース関連api)
   - [選手関連API](#選手関連api)
   - [検索API](#検索api)
...
// 例: 特定日のレース情報を取得
const races = await SwimLiveScraper.getRaceListByGameDate('1234567', '2026-08-01');
console.log(races);
...
- **引数**:
  - `params` (SearchGameParams): 検索パラメータ
